...
class NaiveLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size,num_layer,batch_first):
    ...
    def forward(self, inputs, state):
        ...
        if state is None:
            h = torch.zeros(self.num_layer,batch_size,hidden_size)
            c = torch.zeros(self.num_layer,batch_size,hidden_size)
        else:
            (h, c) = state
            h = h
            c = c
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes):
        ...
        self.lstm = NaiveLSTM(input_size, hidden_size, num_layers,batch_first=True)  # batch_first 为 True则输入输出的数据格式为 (batch, seq, feature)
